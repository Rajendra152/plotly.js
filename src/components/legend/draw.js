...
function _draw(gd, legendObj) {
    ...
    var legendBgData = [
        {
            color: legendObj.bgcolor,
            opacity: legendObj.bgopacity
        },
        {
            color: legendObj.bgendcolor,
            opacity: legendObj.bgendopacity
        }
    ];
    var defsElement = Lib.ensureSingle(legend, 'defs');

    var gradElement = Lib.ensureSingleById(defsElement, 'linearGradient', clipId+'BgGradient');

    var anglePI = (legendObj.bgcolorangle + 270) * (Math.PI / 180);
    var angleCoords = {
    ...
    gradElement.attr(angleCoords);

    var gradStops = gradElement.selectAll('stop').data(legendBgData);
    gradStops.enter().append('stop');

    gradStops.exit().remove();

    gradStops.attr('stop-color', function(d) { return d.color; })
    ...
    bg.call(Color.stroke, legendObj.bordercolor)
        .attr('fill', 'url(#' + clipId + 'BgGradient)')
        .style('stroke-width', legendObj.borderwidth + 'px');
...
function computeTextDimensions(g, gd, legendObj, aTitle) {
    ...
    if(mathjaxNode) {
        ...
        if(aTitle === MAIN_TITLE) {
            ...
            //moving legend text to left of symbol inpixon customisation - symbol placement
             //inpixon customisation - symbol style
             //fixing issue for legend text placement for 'none' symbol style
             var aftertextAttribute =
                    (gd.layout.legend && 
             ...
             var fullLayout = gd._fullLayout;
             var legendTextPaddingLeft = x;
             var isBoxSizingValid =
               boxSizingAttribute >= 0 &&
               legendObj._width < fullLayout.width;

             if (aftertextAttribute === true) {
               legendTextPaddingLeft = constants.itemGap;
               if (isBoxSizingValid) {
                 legendTextPaddingLeft =
                   constants.itemGap + boxSizingAttribute * 0.5; //box sizing edit
               }
               if (isBoxSizingValid && symbolStyleAttribute === "none") {
                 legendTextPaddingLeft =
                   constants.itemGap + boxSizingAttribute * 0.5; //box sizing edit
               }
             } else {
...
function computeLegendDimensions(gd, groups, traces, legendObj) {
    ...
    if(isVertical) {
        ...
        if(isGrouped) {
            ...
            groups.each(function() {
                ...
                // added for inpixon -- Commented so that columns will be shown
    ...
    var additionalBoxSize = parseInt(
        gd.layout.legend && 
        gd.layout.legend.inpixonCustomAttribute && 
        gd.layout.legend.inpixonCustomAttribute.boxsizing && 
        gd.layout.legend.inpixonCustomAttribute.boxsizing.size
      ...
      if (additionalBoxSize >= 0) {
        legendObj._width += additionalBoxSize;
      }
}
