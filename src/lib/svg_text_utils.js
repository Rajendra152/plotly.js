...
var TAG_STYLES = {
    ...
    u: 'text-decoration:underline' // added for inpixon -- for underline property
};
...
exports.makeEditable = function(context, options) {
    ...
    function appendEditable() {
        ...
        // added for inpixon start -- To hide the HTML tags on edit
        var textParts = initialText.split(SPLIT_TAGS)
        var textTags = []

        for(var i=0; i<textParts.length; i++) {
            var match = textParts[i].match(ONE_TAG);
            if(match) textTags.push(match[0]);
        }
        ...
        div.classed('plugin-editable editable', true)
            ...
            .text(context.text()) // added for inpixon
            ...
            .on('blur', function() {
                ...
                // added for inpixon start -- To add the HTML tags that were removed before
                for(var i=0; i<textTags.length/2; i++) {
                    var openingTag = textTags[i];
                    var closingTag = textTags[textTags.length-1-i];
