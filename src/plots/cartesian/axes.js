  ...
  for (
    ...
    if (ax.rangebreaks) {
      if (!axrev) {
        ...
        if (ax.maskBreaks(x) === BADNUM && moveOutsideBreak(x, ax) >= maxRange)
          break;
      }
    }
...
axes.drawOne = function (gd, ax, opts) {
  ...
  var bothTicks = ax.ticks === "both"; // added for inpixon
  ...
  if (ax.ticks) {
    ...
    var mainTickPath = "";
    if (bothTicks) {
      mainTickPath = axes.makeTickPath(
        ax,
        mainLinePosition + ax.ticklen * -tickSigns[2],
        tickSigns[2],
        ax.ticklen + ax.ticklen
      );
    } else {
    ...
    if (ax._anchorAxis && ax.mirror && ax.mirror !== true) {
      // added for inpixon start
      if (bothTicks) {
        mirrorTickPath = axes.makeTickPath(
          ax,
          mainMirrorPosition + ax.ticklen * -tickSigns[3],
          tickSigns[3],
          ax.ticklen + ax.ticklen
        );
      } else {
  ...
  for (i = 0; i < tickSubplots.length; i++) {
    ...
    var spTickPath =
      axes.makeTickPath(ax, linepositions[0], tickSigns[0]) +
  ...
  if (ax.type === "multicategory") {
    ...
    seq.push(function () {
      ...
      return axes.drawLabels(gd, ax, {
        ...
        labelFns: axes.makeLabelFns(
          ax,
          mainLinePosition + standoff * tickSigns[4]
        ),
      });
    });
    ...
    seq.push(function () {
      ax._depth =
        tickSigns[4] * (getLabelLevelBbox("tick2")[ax.side] - mainLinePosition);
    ...
    seq.push(function () {
      ax._depth =
        tickSigns[4] * (getLabelLevelBbox()[ax.side] - mainLinePosition);
    });
  }
...
axes.makeTransTickFn = function (ax) {
  return ax._id.charAt(0) === "x"
    ? function (d) {
       var ticksXAxisPosition = (ax._input && ax._input.axisplacement)
        return strTranslate(ax._offset + ax.l2p(d.x), typeof ticksXAxisPosition === 'undefined'? 0: ticksXAxisPosition); //jerry ticks-x
      }
    : function (d) {
      var ticksYAxisPosition = (ax._input && ax._input.axisplacement)
        return strTranslate(typeof ticksYAxisPosition === 'undefined'? 0:ticksYAxisPosition, ax._offset + ax.l2p(d.x));
      };
};
...
axes.makeTransTickLabelFn = function (ax) {
  ...
  return ax._id.charAt(0) === "x"
    ? function (d) {
      var ticksLableXAxisPosition = (ax._input && ax._input.axisplacement)
        return strTranslate(
          u + ax._offset + ax.l2p(getPosX(d)),
          typeof  ticksLableXAxisPosition === 'undefined'? v:ticksLableXAxisPosition
        );
      }
    : function (d) {
       var ticksLableYAxisPosition = (ax._input && ax._input.axisplacement)
        return strTranslate(
          typeof  ticksLableYAxisPosition=== 'undefined'? v: ticksLableYAxisPosition,
          u + ax._offset + ax.l2p(getPosX(d))
        );
      };
};
...
axes.makeTickPath = function (ax, shift, sgn, len) {
  ...
  if(axLetter === 'x'){
    if(ax._input.ticks === 'both' && ax._input.side === 'counterclockwise'){ // inpixon custimisation major ticks direction both
      return "M0," + (shift + pad * sgn + ax.ticklen/2) + "v" + len * sgn
    }else if(ax._input.side === 'clockwise' && ax._input.ticks === 'both'){
      return "M0," + (shift + pad * sgn + ax.ticklen/2) + "v" + len * -sgn
    }else{
      return "M0," + (shift + pad * sgn) + "v" + len * sgn
    }
  }else{
...
axes.makeLabelFns = function (ax, shift, angle) {
  ...
  if (axLetter === "x") {
    ...
    out.xFn = function (d) { 
      if(ax._name === 'baxis' && ax.ticks === 'ccwOut' || ax._name === 'baxis' && ax.ticks === 'ccwInOut' ){ //inpixon custimisation for ternary ticks
        return d.dx + x0 + xQ * d.fontSize + -ax.ticklen/2;
      }else{
        return d.dx + x0 + xQ * d.fontSize;
      }
    };
    out.yFn = function (d) {
      var inputTicks = (ax._input && ax._input.ticks)
      var inputTicksSide = (ax && ax.side)
      if(inputTicks === 'both' && inputTicksSide === 'bottom'){ 
        return d.dy + y0 + d.fontSize + ax.ticklen * ff;
      }else if(inputTicks === 'both' && inputTicksSide === 'top'){
        return d.dy + y0 + d.fontSize * ff - ax.ticklen; 
      }else if(ax._name === 'baxis' && ax.ticks === 'both' || ax._name === 'baxis' && ax.ticks === 'ccwOut' || ax._name === 'baxis' && ax.ticks === 'ccwInOut' || ax._name === 'baxis' && ax.ticks === '90InOut'){ //inpixon custimisation for ternary ticks
       return d.dy + y0 + d.fontSize * ff + ax.ticklen/2
      }else if(ax._name === 'baxis' && ax.ticks === '90Out'){ //inpixon custimisation for ternary ticks
        return d.dy + y0 + d.fontSize * ff + ax.ticklen;
      }else{
        return d.dy + y0 + d.fontSize * ff ;
      }
    };
    out.xFnforx = function (d) { //inpixon custimisation major ticks direction both
        return d.dx + x0 + xQ * d.fontSize;
      };
    ...
    out.xFn = function (d) {
      //inpixon custimisation
      var inputTicks = (ax._input && ax._input.ticks)
      var inputTicksSide = (ax && ax.side)
      if( inputTicks === 'both' && inputTicksSide === 'left'){
        return d.dx + shift - (x0 + d.fontSize * ff) * flipIt + xQ * d.fontSize - ax.ticklen ;
      }else if(inputTicks === 'both' && inputTicksSide === 'right'){
       return d.dx + shift - (x0 + d.fontSize * ff) * flipIt + xQ * d.fontSize + ax.ticklen;
      }else if(ax._name === 'aaxis' && ax.ticks === 'both'){
        return d.dx + shift - (x0 + d.fontSize * ff) * flipIt + xQ * d.fontSize - ax.ticklen/2;
      }else if(ax._name === 'aaxis' && ax.ticks === '90Out'){
         return -ax.ticklen;
      }else if(ax._name === 'aaxis' && ax.ticks === '90InOut'){
        return -ax.ticklen/2;
      }else if(ax._name === 'aaxis' && ax.ticks === 'ccwOut'){
        return -(d.dy + y0 + d.fontSize * MID_SHIFT + ax.ticklen);
      }else if(ax._name === 'aaxis' && ax.ticks === 'ccwInOut'){
        return -(d.dy + y0 + d.fontSize * MID_SHIFT + ax.ticklen/2);
      }else if(ax._name === 'caxis' && ax.ticks === 'both'){
        return d.dx + shift - (x0 + d.fontSize * ff) * flipIt + xQ * d.fontSize + ax.ticklen/3 + 10;
      }else if(ax._name === 'caxis' && ax.ticks === '90Out' || ax._name === 'caxis' && ax.ticks === 'ccwOut'){
        return d.dx + shift - (x0 + d.fontSize * ff) * flipIt + xQ * d.fontSize + ax.ticklen;
      }else if(ax._name === 'caxis' && ax.ticks === '90InOut' || ax._name === 'caxis' && ax.ticks === 'ccwInOut'){
        return d.dx + shift - (x0 + d.fontSize * ff) * flipIt + xQ * d.fontSize + ax.ticklen/2;
      }else{
       return d.dx + shift - (x0 + d.fontSize * ff) * flipIt + xQ * d.fontSize;
      }        
   };
    //for y-axis both x-direction inpixon custimisation major ticks direction both
    out.xFn1 = function (d) {
      var inputTicks = (ax._input && ax._input.ticks)
      if(inputTicks === 'both'){
        return ( d.dx + ax._mainMirrorPosition - (x0 + d.fontSize * ff) * -1 + xQ * d.fontSize + ax.ticklen);
      }else{
        return ( d.dx + ax._mainMirrorPosition - (x0 + d.fontSize * ff) * -1 + xQ * d.fontSize);
      }
      // 
    };
    //for y-axis both y-direction inpixon custimisation major ticks direction both
      out.yFn1 = function (d) {
        return d.dy + y0 + d.fontSize * MID_SHIFT;
      };
  
      out.yFn = function (d) {
        if(ax._name === 'aaxis' && ax.ticks === 'ccwOut'){
          return -(d.dy + y0 + d.fontSize * MID_SHIFT + ax.ticklen);
        }else if(ax._name === 'aaxis' && ax.ticks === 'ccwInOut'){
          return -(d.dy + y0 + d.fontSize * MID_SHIFT + ax.ticklen/2);
        }else{
...
axes.drawTicks = function (gd, ax, opts) {
  ...
  var pad = (ax.linewidth || 1) / 2;
  var len = ax.ticklen;
  //inpixon custimisation major ticks direction both
  var xaxisTicks = (gd.layout && gd.layout.xaxis && gd.layout.xaxis.ticks)
  var xaxisSide = (gd.layout && gd.layout.xaxis && gd.layout.xaxis.side)
  var yaxisTicks = (gd.layout && gd.layout.yaxis && gd.layout.yaxis.ticks)
  ...
  var sgn =
    ...
    (yaxisSide === "left" &&
      ...
      ? 1
      : -1;
  var shift = ax._mainLinePosition;
  ...
  var ticks = opts.layer
    .selectAll("path." + ax._id === "x" ? "xtick" : "ytick")
  ...
  if (
    ...
    //side:both ticks:inside
    ticks
      .enter()
      .append("path")
      .classed(ax._id === "x" ? "xtick" : "ytick", 1)
      .classed("ticks", 1)
    ...
    ticks
      ...
      .attr(
        ...
        "M0," +
          ...
          len * -sgn * 2 +
          "M0," +
          (ax._mainMirrorPosition - pad + len) +
          "v" +
          len * sgn * 2
      )
      .style("display", null); // visible
    ...
    ticks
      ...
      .call(Color.stroke, ax.tickcolor)
      .style("stroke-width", Drawing.crispRound(gd, ax.tickwidth, 1) + "px")
      .attr("d", "M0," + (shift + pad * sgn) + "v" + len * sgn)
      .style("display", null) // visible
    ...
    ticks
      ...
      .call(Color.stroke, ax.tickcolor)
      .style("stroke-width", Drawing.crispRound(gd, ax.tickwidth, 1) + "px")
      .attr("d", "M0," + (shift + pad - len) + "v" + len * sgn * 2)
      .style("display", null); // visible
  ...
  if (
    ...
    ticks
      ...
      .call(Color.stroke, ax.tickcolor)
      .style("stroke-width", Drawing.crispRound(gd, ax.tickwidth, 1) + "px")
      .attr("d", "M" + (shift + pad * sgn) + ",0h" + len * sgn)
      .style("display", null); // visible
  ...
  if (
    ...
    ticks
      ...
      .call(Color.stroke, ax.tickcolor)
      .style("stroke-width", Drawing.crispRound(gd, ax.tickwidth, 1) + "px")
      .attr("d", "M" + (shift - pad + len) + ",0h" + len * sgn * 2)
      .style("display", null); // visible
    ...
    ticks
      ...
      .call(Color.stroke, ax.tickcolor)
      .style("stroke-width", Drawing.crispRound(gd, ax.tickwidth, 1) + "px")
      .attr("d", "M" + (shift + len - sgn) + ",0h" + len * sgn * 2)
      .style("display", null); // visible
    ...
    //side:both ticks:both hardcoded
    ticks
      .enter()
      .append("path")
      .classed(ax._id === "x" ? "xtick" : "ytick", 1)
      ...
      .attr(
        ...
        "M" +
          ...
          len * sgn * 2 +
          "M" +
          (ax._mainMirrorPosition - len + pad) +
          ",0h" +
          len * -sgn * 2
      )
      .style("display", null); // visible
  ...
  //for rest of graphs like polar radar ternary...
  else {
  ...
  //   .style(
  //     "stroke-width",
  //     Drawing.crispRound(gd, ax.tickwidth, 1) + "px"
  //   )
  //   .attr("d", opts.path)
  //   .style("display", null); // visible
  // hideCounterAxisInsideTickLabels(ax, [TICK_PATH]);
...
axes.drawLabels = function (gd, ax, opts) {
    ...
    //inpixon custimasation major ticks direction both (setting labels according to the side)
    ...
    function positionLabels(s, angle) {
      s.each(function (d) {
        ...
        //inpixon custimisation
        var transform = ax._name === 'aaxis' && ax.ticks === 'both'?
                  opts.transFn.call(thisLabel.node(), d) +(isNumeric(angle) && +angle !== 0? 
                  " rotate(" +angle +"," +  -3.5 +"," +(labelFns.yFn(d) - d.fontSize / 2) +")"
                    : "") :  ax._name === 'caxis' && ax.ticks === 'ccwOut' || ax._name === 'caxis' && ax.ticks === 'ccwInOut' ?
                    opts.transFn.call(thisLabel.node(), d) +(isNumeric(angle) && +angle !== 0? 
                  " rotate(" +angle +"," + 3.5 +"," +(labelFns.yFn(d) - d.fontSize / 2) +")"
                    : "") :ax._name === 'aaxis' && ax.ticks === 'ccwInOut' ?
                    opts.transFn.call(thisLabel.node(), d) +(isNumeric(angle) && +angle !== 0? 
                  " rotate(" +angle +"," + -3.5 +"," +(labelFns.yFn(d)) +")"
                    : "") :ax._name === 'caxis' && ax.ticks === 'both'?
                    opts.transFn.call(thisLabel.node(), d) +(isNumeric(angle) && +angle !== 0? 
                    " rotate(" +angle +"," +labelFns.xFn(d) * 2 +"," +(labelFns.yFn(d) - d.fontSize / 2) +")"
                      : ""): ax._name === 'aaxis' && ax.ticks === 'ccwOut'? 
                      opts.transFn.call(thisLabel.node(), d) +(isNumeric(angle) && +angle !== 0? 
                    " rotate(" +angle +"," + -3.5 +"," +(labelFns.yFn(d)) +")"
                      : "")
                    : opts.transFn.call(thisLabel.node(), d) +(isNumeric(angle) && +angle !== 0? 
                    " rotate(" +angle +"," +labelFns.xFn(d) +"," +(labelFns.yFn(d) - d.fontSize / 2) +")"
                      : "")
        ...
        if (mathjaxGroup.empty()) {
          ...
          mathjaxGroup.attr(
            "transform",
            transform + strTranslate(mjShift, 0)
          );
        }
      });
    }
    ...
    ax._adjustTickLabelsOverflow = function () {
      ...
      for (var subplot in fullLayout._plots) {
        ...
        if (
          ax._id !== plotinfo.xaxis._id &&
          ax._id !== plotinfo.yaxis._id
        )
          continue;
    ...
    ax._hideCounterAxisInsideTickLabels = function (partialOpts) {
      ...
      anchoredAxes.forEach(function (anchorAx, idx) {
        if (anchorAx && insideTicklabelposition(anchorAx)) {
          (
            partialOpts || [ZERO_PATH, GRID_PATH, TICK_PATH, TICK_TEXT]
          ).forEach(function (e) {
            var isPeriodLabel =
              e.K === "tick" &&
              e.L === "text" &&
            ...
            var sel;
            if (e.K === ZERO_PATH.K)
              sel = mainPlotinfo.zerolinelayer.selectAll(
                "." + ax._id + "zl"
              );
            ...
            sel.each(function () {
              ...
              w.each(function (d) {
                var q =
                  ax.l2p(isPeriodLabel ? getPosX(d) : d.x) + ax._offset;
    ...
    if (
      ...
      var tickLabelsboth = opts.layer
        .selectAll("g." + ax._id === "x" ? "xtick" : "ytick")
        .data(ax.showticklabels ? vals : [], tickDataFn);
      var tickAngle = opts.secondary ? 0 : ax.tickangle;
      var prevAngle = (ax._prevTickAngles || {})[cls];
      ...
      if (
        ...
        tickLabelsboth
          ...
          .each(function (d) {
            var thisLabel = d3.select(this);
            var newPromise = gd._promises.length;
            var labelStandoff = labelFns.labelStandoff;
            if (ax._id === "x") {
              var inputTicks = (ax._input && ax._input.ticks)
              thisLabel
                .call(
                  svgTextUtils.positionText,
                  labelFns.xFnforx(d),
                  inputTicks === 'both'? ax._mainMirrorPosition + labelStandoff  * -1.5 - ax.ticklen: ax._mainMirrorPosition + labelStandoff * -1.5 //ticklen
                )
                .call(Drawing.font, d.font, d.fontSize, d.fontColor)
                .text(d.text)
                .call(svgTextUtils.convertToTspans, gd);
        ...
        hideCounterAxisInsideTickLabels(ax, [TICK_TEXT]);
        if (opts.repositionOnUpdate) {
          tickLabelsboth.each(function (d) {
            d3.select(this)
              .select("text")
              .call(
                svgTextUtils.positionText,
      ...
      function positionLabels(s, angle) {
        s.each(function (d) {
          ...
          if (mathjaxGroup.empty()) {
            ...
            if (ax._input.side === "both") {
              thisText.style("opacity", 1); 
            } else if (
              ax._input.side === "bottom" ||
              ax._input.side === "top" ||
              ax._input.side === "left" ||
              ax._input.side === "right"
            ) {
          ...
          if (mathjaxGroup.empty()) {
            ...
            if (isX) {
              ...
              if (bb.bottom > max) adjust = 1;
                ...
                bb.top + (ax.tickangle ? 0 : d.fontSize / 4) <
                min
              )
                adjust = 1;
            }
            ...
            if (adjust) {
              ...
              t.style("opacity", 4); // visible bottom enters
      ...
      positionLabels(
        tickLabelsboth,
        prevAngle + 1 ? prevAngle : tickAngle
      );
      ...
      function fixLabelOverlaps() {
        ...
        if (
          ...
          if (
            (ax.tickson === "boundaries" || ax.showdividers) &&
            !opts.secondary
          ) {
            var gap = 2;
            ...
            var tickSpacing =
              Math.abs((vals[vLen - 1].x - vals[0].x) * ax._m) /
              (vLen - 1);
            ...
            for (i = 0; i < lbbArray.length - 1; i++) {
              if (
                Lib.bBoxIntersect(lbbArray[i], lbbArray[i + 1], pad)
              ) {
