...
function makeSubplotLayer(gd, plotinfo) {
    ...
    if(!plotinfo.mainplot) {
        if(hasOnlyLargeSploms) {
            ...
            //Inpixon customisation - grid layering customization

            var abovegridAttribute = (gd.layout.legend && 
                gd.layout.legend.inpixonCustomAttribute && 
                gd.layout.legend.inpixonCustomAttribute.layering && 
                gd.layout.legend.inpixonCustomAttribute.layering.abovegrid);
  
              
              if (
                  abovegridAttribute === true
                 ){
                    plotgroup.select("g.plot").remove();
                    plotgroup.select("g.gridlayer").remove();
    ...
    .attr('transform' , `translate(0,${typeof gd.layout.xaxis.axisplacement === 'undefined'? 0:gd.layout.xaxis.axisplacement})`); 

    plotinfo.ylines.style("fill", "none").classed("crisp", true)
    .attr('transform' , `translate(${typeof gd.layout.yaxis.axisplacement === 'undefined'? 0:gd.layout.yaxis.axisplacement},0)`); 
}
