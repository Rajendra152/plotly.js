...
proto.updateAngularAxis = function(fullLayout, polarLayout) {
    ...
    labelFns.xFn = function (d) {
        var rad = t2g(d); 
        var polarTicks = (gd.layout && gd.layout.polar && gd.layout.polar.angularaxis && gd.layout.polar.angularaxis.ticks) //inpixon custimization major ticks direction both 
        if(polarTicks=== 'both'){
          return Math.cos(rad) *  ax.ticklen/3 * labelStandoff;
        }else{
      ...
      labelFns.yFn = function (d) {
        ...
        if(polarTicks === 'both'){
          return (
            -Math.sin(rad) * (labelStandoff * ax.ticklen/4 + d.fontSize * ff) +
            Math.abs(Math.cos(rad)) * (d.fontSize * MID_SHIFT)
          );
        }else{
        return (
          -Math.sin(rad) * (labelStandoff + d.fontSize * ff) +
          Math.abs(Math.cos(rad)) * (d.fontSize * MID_SHIFT)
        );
        }
      };
    ...
    if(ax.visible) {
        if(gd.layout.polar.angularaxis.ticks === 'both'){ //inpixon custimization major ticks direction both 
            var tickSign = ax.ticks === "inside" ? -1 : 1; 
            var pad = (ax.linewidth || 1) / 2;
          Axes.drawTicks(gd, ax, {
            vals: vals,
            layer: layers["angular-axis"],
            path: "M" + -tickSign * ax.ticklen/2 + ",0h" + tickSign * ax.ticklen,
            transFn: transFn2,
            crisp: false,
          });
          }
          ...
          Axes.drawTicks(gd, ax, {
            vals: vals,
            layer: layers["angular-axis"],
            path: "M" + tickSign + ",0h" + tickSign * ax.ticklen,
            transFn: transFn2,
            crisp: false,
          });
