...
function plot(gd, plotinfo, cdbox, boxLayer) {
    ...
    Lib.makeTraceGroups(boxLayer, cdbox, 'trace boxes').each(function(cd) {
        ...
        var t = cd0.t;
        var trace = cd0.trace;
        var meanstyle = trace.meanstyle;
        var isHorizontal = trace.orientation === 'h';
        if(isHorizontal) {
          if(meanstyle.meanalign=='center'){
            plotGroup.style('transform', 'translate(0px, 0px)')
          }
          if(meanstyle.meanalign=='left'){
            plotGroup.style('transform', 'translate(0px, -10px)')
          }
          if(meanstyle.meanalign=='right'){
            plotGroup.style('transform', 'translate(0px, 10px)')
          }
        }else{
          if(meanstyle.meanalign=='center'){
          ...
          if(meanstyle.meanalign=='left'){
            plotGroup.style('transform', 'translate(-10px, 0px)')
          }
          if(meanstyle.meanalign=='right'){
            plotGroup.style('transform', 'translate(10px, 0px)')
          }
...
function plotBoxMean(sel, axes, trace, t) {
    var valAxis = axes.val;
    var posAxis = axes.pos;
    var meanstyle = trace.meanstyle; // added for inpixon
    var posHasRangeBreaks = !!posAxis.rangebreaks;
    ...
    var pathsgap = sel.selectAll('path.meangap').data((
        (trace.type === 'box' && trace.boxmean) ||
        (trace.type === 'violin' && trace.box.visible && trace.meanline.visible)
    ) ? Lib.identity : []);
    
    pathsgap.enter().append('path').attr('class', 'meangap')
        .style({
            fill: 'none',
            'vector-effect': 'non-scaling-stroke',
            'stroke': meanstyle.meangapcolor,
            'stroke-dasharray': '0px',
            'stroke-width': meanstyle.meanwidth,
            'opacity': meanstyle.meanopacity,
        });
    pathsgap.exit().remove();
    ...
    paths.enter().append('path')
        ...
        .style({
            fill: 'none',
            'vector-effect': 'non-scaling-stroke',
            'stroke': meanstyle.meancolor,
            'stroke-dasharray': meanstyle.meanpad,
            'stroke-width': meanstyle.meanwidth,
            'opacity': meanstyle.meanopacity,
    })
    paths.exit().remove();
